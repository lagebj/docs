...
1. One bounded content slice at a time.
2. Do not rewrite multiple top-level sections in one slice.
3. Reviewer must be a separate tool/session from the editor.
4. Verifier trusts only executed command output.
5. Changes must preserve existing navigation coherence unless the plan explicitly includes navigation edits.
6. Writing must follow CONTRIBUTING.md guidance:
   - active voice
   - direct address
   - concise sentences
